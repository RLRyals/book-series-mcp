-- Migration: Fix ROUND function with double precision issue
-- Date: 2025-07-11

-- The error "function round(double precision, integer) does not exist" is happening because
-- we need to make sure we're using the right version of ROUND that PostgreSQL supports.
-- PostgreSQL has ROUND for numeric type but needs proper casting for double precision.

-- Create a utility function to handle rounding of double precision values correctly
CREATE OR REPLACE FUNCTION round_double_precision(value double precision, scale integer)
RETURNS numeric AS $$
BEGIN
    RETURN ROUND(value::numeric, scale);
END;
$$ LANGUAGE plpgsql;

-- Update the getWritingProgress function in the writing-server to use numeric casting instead
-- (This is a comment for reference - the actual fix will be in the JavaScript code)

-- Also update any views that may be using ROUND with double precision to use proper casting
...
-- Recreate views with correct ROUND usage
...
SELECT 
    ...
    ROUND((COALESCE(SUM(c.word_count), 0)::numeric / NULLIF(b.target_word_count, 0)::numeric) * 100.0, 2) as completion_percentage
FROM books b
...
SELECT 
    ...
    b.editing_complete,
    ROUND((COALESCE(b.word_count, 0)::numeric / NULLIF(b.target_word_count, 0)::numeric) * 100.0, 2) as completion_percentage,
...
SELECT 
    ...
    c.word_count,
    c.target_word_count,
    ROUND((COALESCE(c.word_count, 0)::numeric / NULLIF(c.target_word_count, 0)::numeric) * 100.0, 2) as completion_percentage,
    CASE 
