-- Migration: Add daily word goal and ensure consistent ROUND usage
-- Date: 2025-07-02

-- Add daily_word_goal column to writing_goals table
ALTER TABLE writing_goals 
ADD COLUMN daily_word_goal INTEGER;

-- Update all analytics views to ensure consistent ROUND usage
...
SELECT 
    ...
    ROUND(CAST(COALESCE(SUM(c.word_count), 0) AS numeric) / NULLIF(b.target_word_count, 0) * 100, 2) as completion_percentage
FROM books b
...
SELECT 
    ...
    b.editing_complete,
    ROUND(CAST(b.word_count AS numeric) / NULLIF(b.target_word_count, 0) * 100, 2) as completion_percentage,
    COUNT(ws.id) as total_sessions,
    COALESCE(SUM(ws.words_written), 0) as total_words_logged,
    ROUND(CAST(AVG(ws.productivity_rating) FILTER (WHERE ws.productivity_rating IS NOT NULL) AS numeric), 2) as avg_productivity,
    ROUND(CAST(AVG(ws.words_written) FILTER (WHERE ws.words_written > 0) AS numeric), 0) as avg_words_per_session,
...
SELECT 
    ...
    c.word_count,
    c.target_word_count,
    ROUND(CAST(c.word_count AS numeric) / NULLIF(c.target_word_count, 0) * 100, 2) as completion_percentage,
    CASE 
